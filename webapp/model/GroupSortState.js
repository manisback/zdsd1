sap.ui.define([
	...
	return BaseObject.extend("zdsd.heineken01.model.GroupSortState", {
		 ...
		 * @alias zdsd.heineken01.model.GroupSortState
		 */
		...
		/**
		 * Sorts by SalesOrderNumber, or by NetPriceAmount
		 *
		...
		/**
		 * Groups by NetPriceAmount, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "NetPriceAmount") {
				// Grouping means sorting so we set the select to the same Entity used for grouping
				this._oViewModel.setProperty("/sortBy", "NetPriceAmount");

				aSorters.push(
					new Sorter("NetPriceAmount", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "SalesOrderNumber");
			}
